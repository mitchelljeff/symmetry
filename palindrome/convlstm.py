...
if __name__ == '__main__':
    ...
    with graph.as_default():
        ...
        wf=tf.get_variable("wf", shape=[emb_size+units, units*3], initializer=tf.contrib.layers.xavier_initializer())
        bf=tf.get_variable("bf", shape=[1,units*3], initializer=tf.contrib.layers.xavier_initializer())
        ...
        up  =tf.stack([tf.eye(units,dtype=tf.float32),tf.zeros([units,units],dtype=tf.float32),tf.zeros([units,units],dtype=tf.float32)],axis=0)
        stay=tf.stack([tf.zeros([units,units],dtype=tf.float32),tf.eye(units,dtype=tf.float32),tf.zeros([units,units],dtype=tf.float32)],axis=0)
        down=tf.stack([tf.zeros([units,units],dtype=tf.float32),tf.zeros([units,units],dtype=tf.float32),tf.eye(units,dtype=tf.float32)],axis=0)


        embedded=tf.gather(e,tfseq)

        h     = tf.zeros([tfbs,units],dtype=tf.float32)
        state = state = tf.zeros([tfbs,stacksize,units],dtype=tf.float32)
        inputs=tf.unstack(embedded,axis=1)
        outputs=[None]*max_seq
        for i in range(max_seq):
            hx=tf.concat([h,inputs[i]],axis=1)
            f=tf.matmul(hx,wf)+bf
            f=tf.nn.softmax(tf.reshape(f,[-1,1,units,3]))
            it=tf.sigmoid(tf.matmul(hx,wi)+bi)
            c=tf.tanh(tf.matmul(hx,wc)+bc)
            o=tf.sigmoid(tf.matmul(hx,wo)+bo)
            state1 = tf.nn.conv1d(state,filters=up  ,stride=1,padding="SAME") 
            state1 = state1 + tf.concat([tf.expand_dims(inputs[i],axis=1),tf.zeros([tfbs,stacksize-1,units],dtype=tf.float32)],axis=1)
            state2 = tf.nn.conv1d(state,filters=stay,stride=1,padding="SAME")
            state3 = tf.nn.conv1d(state,filters=down,stride=1,padding="SAME")
            state = tf.stack([state1,state2,state3],axis=-1)
            state = tf.reduce_sum(tf.multiply(state,f),axis=-1)
            h = tf.multiply(tf.tanh(state[:,0,:]),o)
