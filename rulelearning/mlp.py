...
with graph.as_default():
    ...
    w1=tf.get_variable("w1", shape=[1,vsize,seqlen,nhid], initializer=tf.contrib.layers.xavier_initializer())
    w2=tf.get_variable("w2", shape=[1,nhid,2], initializer=tf.contrib.layers.xavier_initializer())

    hid=tf.sigmoid(tf.reduce_sum(tf.multiply(tf.expand_dims(tfseq,-1),w1),axis=[1,2]))

    logits=tf.reduce_sum(tf.multiply(tf.expand_dims(hid,-1),w2),axis=1)
...
with tf.Session(graph=graph) as sess:
    ...
    for epoch,batch,bs in batcher(train,epochs,batch_size):
        feed_dict={tfseq:batch["seq"], tflabel:batch["label"]}
        ...
        if epoch==epochs-1:
            ...
            for depoch,dbatch,dbs in batcher(test,1,batch_size):
                feed_dict={tfseq:dbatch["seq"], tflabel:dbatch["label"]}
